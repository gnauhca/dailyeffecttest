...
export default class ChessboardActor extends PlayActor {
  init() {
    ...
    this.pieceData = (new Array(this.vertical)).fill(1).map((a) => new Array(this.horizontal));
  ...
  isWin(piece, crood) {
    ...
    const calPiece = (crood, step, piece) => {
      let pCount = 0;
    ...
    for (let i = 0; i < directions.length; i++) {
      ...
      let step;
      const curCrood = { x: Number(crood.x), y: Number(crood.y) }; // 当前判断的子
