...
const planeFunctions = {
  ...
  'sin(x^2+y^2)': function (x, z, offset) {
    return Math.sin(Math.sqrt((x / 2) ** 2 + (z / 2) ** 2) - offset);
  },
};
...
function bannerInit(cvs) {
  const ctx = cvs.getContext('2d');

  const scene = new F3.Scene();
  const camera = new F3.Camera();
  camera.origin = new F3.Vector3(cvs.width / 2, cvs.height / 3);
  camera.p = 800;

  const renderer = new F3.Renderer(ctx, cvs);
  const effect = new Effect(renderer, scene, camera, cvs);
  effect.animate();

  const functions = document.querySelector('.functions');
  let btnHTML = '';
  for (const name in planeFunctions) {
    btnHTML += `<div class="btn" data-function="${name}">${name}</div>`;
  }
  functions.innerHTML = btnHTML;

  const btns = functions.querySelectorAll('.btn');
  function selectFunction(funName) {
    btns.forEach((btn) => {
      const dataFunction = btn.dataset.function;
  ...
  const rotateBtns = rotate.querySelectorAll('.btn');
  function toggleRotate(_r) {
    rotateBtns.forEach((rotateBtn) => {
      const r = rotateBtn.dataset.rotate;
      if (r === _r) {
