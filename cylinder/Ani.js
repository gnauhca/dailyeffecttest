...
const directionLight = new C3.DirectionLight(new C3.Vector3(0, 0, 1), 1);
...
for (let i = 0; i < items.length; i++) {
  ...
  face.lightHandler = function (elem, brightness) {
    const percent = Math.max(brightness, 0.5).toFixed(2);
  ...
  spin() {
    ...
    const tween = new TWEEN.Tween(tweenObj)
      ...
      .onUpdate((obj) => {
        ...
        faces[index].setScale(obj.scale, obj.scale, obj.scale);
      })
      .onComplete(() => {
        this.spinTween = null;
      })
