...
class Branch {
  constructor(tree, parent, options) {
    ...
    // 树枝长度
    const totalLength = Branch.maxLength * (Branch.maxDeep - this.deep) / Branch.maxDeep;
  ...
  grow(delta) {
    ...
    if (this.progress === 1) {
      if (this.endAtPercent < 1) {
        ...
        // 生出两个子级树枝
        const leftAnglesAppend = [
          (0.4 + 1.3 * Math.random()) * Math.PI / Math.min(this.deep + 1, 3),
          (0.4 + 1.3 * Math.random()) * Math.PI / 4 / Math.min(this.deep * 1.5 + 0.1),
        ];
        const leftStartAtPercent = 0;
        const leftEndAtPercent = Math.random() > 0.5 ? 1 : Math.random() * 0.5 + 0.2;
        const leftBranch = new Branch(this.tree, this, {
          anglesAppend: leftAnglesAppend,
          leftStartAtPercent,
          leftEndAtPercent,
        });
        const rightAnglesAppend = [
          (0.4 + 1.3 * Math.random()) * -Math.PI / Math.min(this.deep + 1, 3),
          (0.4 + 1.3 * Math.random()) * Math.PI / 4 / Math.min(this.deep * 1.5 + 0.1),
        ];
        const rightStartAtPercent = 0;
        const rightEndAtPercent = Math.random() > 0.5 ? 1 : Math.random() * 0.5 + 0.2;
        const rightBranch = new Branch(this.tree, this, {
...
class Tree {
  constructor(options, camera) {
    ...
    const rootBranch = new Branch(this, {
      // 根的 parent，用于辅助
  ...
  addBranch(branch) { // return;
    setTimeout(() => {
      ...
      const parentPointsIndexes = this.getPointsIndexes(branch.parent.index);
      const branchPointsIndexes = this.getPointsIndexes(branch.index);
  ...
  getSurroundPoint(center, vector, cameraPosition, redius) {
    const point1 = new THREE.Vector3().crossVectors(
  ...
  grow(delta) {
    this.branches.forEach((branch) => {
      ...
      const thickness = (this.thickness - branch.deep) ** 2;
      const { vector } = branch;
      const center = branch.end;
      const branchPointsIndexes = this.getPointsIndexes(branch.index);
      const surroundPoint = this.getSurroundPoint(center, vector, this.camera.position, thickness);
