...
const getSurroundPoint = (function () {
  const cache = {};
  ...
  return function (center, vector, radius, pointNum) { // console.log(vector.clone().normalize());
    ...
    const cross = new THREE.Vector3().crossVectors(up, vector).normalize();
    ...
    // mesh.updateMatrixWorld();
    const vertices = mesh.geometry.vertices.slice(0, pointNum).map((v) => v.clone().applyAxisAngle(cross, angle).setLength(radius));
    return vertices;
  };
}());
...
class Branch {
  constructor(tree, parent, options) {
    ...
    const progressSegment = this.endAtPercent - this.startAtPercent;
  ...
  grow(delta) {
    ...
    if (this.endAtPercent < 0.7) {
      ...
      const startAtPercent = this.endAtPercent;
      const endAtPercent = Math.min(startAtPercent + 0.3 + Math.random(), 1);
      const percentSegment = endAtPercent - startAtPercent;
      const { branchLength } = this;
      const length = branchLength * percentSegment;
      const startRadius = this.endRadius;
      const endRadius = startRadius - this.tree.radiusReduceSpeed * length;
      const { speed } = this;

      const sameDeepBranch = new Branch(this.tree, this, {
        angles,
        startAtPercent,
        endAtPercent,
        percentSegment,
      ...
      // 直系 connected
      const cOptions = this.generateChildBaseOptions();
      const cstartRadius = this.endRadius;
      const cendRadius = Math.max(cstartRadius - this.tree.radiusReduceSpeed * cOptions.length, 0.2);
      const cAngles = [
        this.angles[0] + 20 * RADIAN * sign1 * -1,
        this.angles[1] + 10 * RADIAN * sign2 * -1,
      ];
      const cBranch = new Branch(this.tree, this, ({
        angles: cAngles,
      ...
      // 非直系 isolated
      const iOptions = this.generateChildBaseOptions();
      const istartRadius = this.endRadius * (0.7 + Math.random() * 0.3);
      const iendRadius = Math.max(istartRadius - this.tree.radiusReduceSpeed * iOptions.length, 0.2);
      const iAngles = [
        this.angles[0] + 40 * RADIAN * sign3,
        this.angles[1] + -30 * RADIAN * sign4 * -1,
      ];
      const iBranch = new Branch(this.tree, this, ({
        angles: iAngles,
  ...
  generateChildBaseOptions() {
    const length = this.tree.maxDeep * this.tree.maxLength * (0.5 + 0.5 * Math.random()) / (this.deep ** 1.5 + 1 + this.tree.maxDeep);
    const startAtPercent = 0;
    const endAtPercent = Math.random() > 0.5 ? Math.random() * 0.3 + 0.3 : 1;
...
class Tree {
  constructor(options, camera) {
    ...
    const rootBranch = new Branch(this, null, {
      isIsolate: true,
      angles: [0, 90 * RADIAN],
  ...
  addBranch(branch) {
    setTimeout(() => {
      ...
      const endPointIndex = this.treeGeom.vertices.length;
      const endPointNum = this.getPointNumByBranchRadius(branch.endRadius);
  ...
  getPointsIndexes(branchIndexes) {
    const branchFirstPointIndex = (branchIndexes + 1) * 2;
    const branchPointsIndexes = [branchFirstPointIndex, branchFirstPointIndex + 1];
  ...
  grow(delta) {
    this.branches.forEach((branch, i) => {
      ...
      const pointInfo = this.branchesPointInfo[branch.id];
      let { vector } = branch;
...
class Ani extends Time {
  constructor() {
    ...
    const tree1 = new Tree({ color: 0x23498f }, this.camera);
    const tree2 = new Tree({ color: 0x3dafff }, this.camera);
